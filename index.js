...
//Obtener las opciones de los templates: npm-js, npm-ts
...
inquirer.prompt(QUESTIONS).then(respuestas => {
    ...
    createDirectoriesFileContent(templatePath, proyecto)
...
function createDirectoriesFileContent(templatePath, projectName) {
    ...
    listFileDirectories.forEach(item => {
        ...
        if (stats.isFile() ){
            ...
            createDirectoriesFileContent(path.join(templatePath, item), path.join(projectName, item))
        }
    })
}
